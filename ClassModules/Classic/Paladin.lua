...
function Paladin:ClassInit()
	...
	for k, v in pairs(threatValues.greaterBlessing) do
		self.CastLandedHandlers[k] = self.greaterBless
	end
	for k, v in pairs(threatValues.lesserBlessing) do
		self.CastLandedHandlers[k] = self.lesserBless
	end
	-- Righteous Fury
...
function Paladin:lesserBless(spellID, recipient)
	result = self:RighteousFury(threatValues.lesserBlessing[spellID])
	self:AddThreat(result)
end

function Paladin:greaterBless(spellID, recipient, spellName )
